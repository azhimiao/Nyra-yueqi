/**
 * YEOS package unpack — shared zip handling for all kinds
 */

import { unzipSync, strFromU8 } from "fflate";
import { validateAnyManifest } from "./manifest-schema.js";
import { KIND_PHONE_EXT } from "./kinds.js";
...
function kindMessage(kind) {
  if (kind === KIND_PHONE_EXT) return "安装包无法识别，请检查是否为栖机扩展包";
  return "安装包无法识别，请检查是否为有效的 YEOS 安装包";
}

/**
 * @param {Uint8Array} bytes
 */
export async function unpackYeosPackage(bytes) {
  let entries;
  try {
    entries = unzipSync(bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes));
  } catch {
    return { ok: false, code: "missing_manifest", message: "安装包无法识别，请检查 zip 是否损坏" };
  }

  const manifestKey = findManifestEntry(entries);
  if (!manifestKey) {
    return { ok: false, code: "missing_manifest", message: "缺少 manifest.json" };
  }
  ...
  const validated = validateAnyManifest(manifestRaw);
  if (!validated.ok) {
    return {
      ...validated,
      message: validated.message || kindMessage(manifestRaw?.kind),
    };
  }
  ...
  const entry = validated.manifest.entry;
  if (!files[entry]) {
 ...
 */
export async function unpackYeosFile(file) {
  const buf = await file.arrayBuffer();
  return unpackYeosPackage(new Uint8Array(buf));
}
